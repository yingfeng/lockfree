 ...
 * been retired. This information can be used for the hazard pointer scheme
 * to determine whether a hazard pointer can be acquired.
 ...
 * the tree, and, hence, so might v. For more information about how we use
 * marks to decide whether nodes are in the tree, and about the complications
 * that arise due to this technique, see the paper "Reclaiming memory for
 * lock-free data structures: there has to be a better way."
 */
...
#define BST_REPLACE_ALLOCATED_SCXRECORD(tid) BST_GET_ALLOCATED_SCXRECORD_PTR(tid) = bst_allocateSCXRecord(tid)
#define BST_GET_ALLOCATED_SCXRECORD_PTR(tid) allocatedSCXRecord[tid*PREFETCH_SIZE_WORDS]

    // similarly, allocatedNodes[tid*PREFETCH_SIZE_WORDS+i] = an allocated node
    //     for i = 0..MAX_NODES-2
    Node<K,V> **allocatedNodes;
#define BST_REPLACE_ALLOCATED_NODE(tid, i) BST_GET_ALLOCATED_NODE_PTR(tid, i) = bst_allocateNode(tid)
#define BST_GET_ALLOCATED_NODE_PTR(tid, i) allocatedNodes[tid*(PREFETCH_SIZE_WORDS+MAX_NODES-1)+i]
    
    bool reclaimMemoryAfterSCX( // for DEBRA and DEBRA+
            const int tid,
    ...
    bool updateInsert(const int, const K& key, const V& val, const bool onlyIfAbsent, V *result); // last arg is an output args
    bool updateErase(const int, const K& key, V *result); // last arg is an output args
    ...
    ~BST() {
        ...
        for (int tid=0;tid<recordmgr->NUM_PROCESSES;++tid) {
            for (int i=0;i<MAX_NODES-1;++i) {
                recordmgr->deallocate(tid, BST_GET_ALLOCATED_NODE_PTR(tid, i));
            }
            recordmgr->deallocate(tid, BST_GET_ALLOCATED_SCXRECORD_PTR(tid));
        }
        delete recordmgr;
        delete counters;
