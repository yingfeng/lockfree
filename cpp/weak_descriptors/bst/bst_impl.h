...
Node<K,V>* bst<K,V,Compare,RecManager>::allocateNode(
            const int tid) {
    this->recmgr->getDebugInfo((Node<K,V>*) NULL)->addToPool(tid, 1);
...
bool bst<K,V,Compare,RecManager>::scx(
    ...
    newdesc->newNode = newNode;
    for (int i=0;i<info->numberOfNodes;++i) {
        newdesc->nodes[i] = info->nodes[i];
    }
    for (int i=0;i<info->numberOfNodesToFreeze;++i) {
        newdesc->scxRecordsSeen[i] = (SCXRecord<K,V> *) info->llxResults[i];
    }
    // note: we do not need to insert an explicit barrier after these writes,
    // since a CAS will occur before this object becomes reachable
    // from an entry point to the data structure.
    newdesc->field = field;
    newdesc->numberOfNodes = (char) info->numberOfNodes;
    newdesc->numberOfNodesToFreeze = (char) info->numberOfNodesToFreeze;
    ...
    SOFTWARE_BARRIER;
    int state = help(tid, TAGPTR1_NEW(tid, newdesc->mutables), newdesc, false);
...
template <class K, class V, class Compare, class RecManager>
void bst<K,V,Compare,RecManager>::helpOther(const int tid, tagptr_t tagptr) {
...
template <class K, class V, class Compare, class RecManager>
int bst<K,V,Compare,RecManager>::help(const int tid, tagptr_t tagptr, SCXRecord<K,V> *snap, bool helpingOther) {
    ...
    //  by the owner)
    for (int i=helpingOther; i<snap->numberOfNodesToFreeze; ++i) { // freeze sub-tree
        if (snap->scxRecordsSeen[i] == LLX_RETURN_IS_LEAF) {
            TRACE COUTATOMICTID("nodes["<<i<<"] is a leaf");
            assert(i > 0); // nodes[0] cannot be a leaf...
            continue; // do not freeze leaves
        }
        
        uintptr_t exp = (uintptr_t) snap->scxRecordsSeen[i];
        bool successfulCAS = snap->nodes[i]->scxRecord.compare_exchange_strong(exp, tagptr); // MEMBAR ON X86/64
    ...
    for (int i=1; i<snap->numberOfNodesToFreeze; ++i) {
        if (snap->scxRecordsSeen[i] == LLX_RETURN_IS_LEAF) continue; // do not mark leaves
        snap->nodes[i]->marked.store(true, memory_order_relaxed); // finalize all but first node
    }
    
    // CAS in the new sub-tree (update CAS)
    uintptr_t expected = (uintptr_t) snap->nodes[1];
    snap->field->compare_exchange_strong(expected, (uintptr_t) snap->newNode);                             // MEMBAR ON X86/64
    ...
    MUTABLES1_WRITE_FIELD(ptr->mutables, snap->mutables, SCXRecord<K comma1 V>::STATE_COMMITTED, MUTABLES1_MASK_STATE, MUTABLES1_OFFSET_STATE);
...
void * bst<K,V,Compare,RecManager>::llx(
    ...
    bool succ;
    int state = DESC1_READ_FIELD(succ, TAGPTR1_UNPACK_PTR(tagptr1)->mutables, tagptr1, MUTABLES1_MASK_STATE, MUTABLES1_OFFSET_STATE);
    ...
    if ((state & SCXRecord<K,V>::STATE_COMMITTED && !marked) || state & SCXRecord<K,V>::STATE_ABORTED) {
        ...
        *retRight = (Node<K,V>*) node->right.load(memory_order_relaxed);
        ...
        if (tagptr1 == tagptr2) {
            ...
            DEBUG2 counters->llxSuccess->inc(tid);
    ...
    DEBUG2 counters->llxFail->inc(tid);
    return NULL;            // fail
}
