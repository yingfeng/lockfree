...
private:
    ...
    bslack_SCXRecord<DEGREE,K> * DUMMY;
    bslack_SCXRecord<DEGREE,K> * FINALIZED;
    bslack_SCXRecord<DEGREE,K> * FAILED;
    ...
    /* 2-bit state | 14-bit highest index reached | 14-bit number of internal nodes frozen = total 30 bits */
    #define ABORT_STATE_INIT(i, numFrozen) (bslack_SCXRecord<DEGREE,K>::STATE_ABORTED | ((i)<<2) | ((numFrozen)<<16))
    #define STATE_FROZEN_NODE_INCREMENT (1<<16)
    #define STATE_GET_HIGHEST_INDEX_REACHED(state) (((state) & (((1<<14)-1)<<2))>>2)
    #define STATE_GET_NUMBER_OF_NODES_FROZEN(state) (((state) & (((1<<14)-1)<<16))>>16)
...
private:
    ...
    int help(const int tid, bslack_SCXRecord<DEGREE,K> *scx, bool helpingOther);
    ...
    bslack_SCXRecord<DEGREE,K>* allocateSCXRecord(const int tid);
    bslack_Node<DEGREE,K>* allocateNode(const int tid);
    
    bool tryRetireSCXRecord(const int tid, bslack_SCXRecord<DEGREE,K> * const otherSCX);
    void reclaimMemoryAfterSCX(const int tid, bslack_wrapper_info<DEGREE,K> * info, bslack_SCXRecord<DEGREE,K>* scxPtr);
    
    void freeSubtree(bslack_Node<DEGREE,K>* node, int* nodes, int* scxRecords, set<void*>* scxPtrsSeen) {
        const int tid = 0;
        if (node == NULL) return;
        if (!node->isLeaf()) {
            for (int i=0;i<node->getABDegree();++i) {
                freeSubtree(node->ptrs[i], nodes, scxRecords, scxPtrsSeen);
            }
            bslack_SCXRecord<DEGREE,K>* rec = node->scxPtr;
            if (rec != DUMMY && scxPtrsSeen->find(rec) == scxPtrsSeen->end()) {
                ++(*scxRecords);
                scxPtrsSeen->insert(rec);
                recordmgr->retire(tid, rec);
            }
        }
        ++(*nodes);
...
public:
    ...
    , counters(new debugCounters(numProcesses)) {
        ...
        DUMMY->state = bslack_SCXRecord<DEGREE,K>::STATE_COMMITTED;
        FAILED = allocateSCXRecord(tid);
        FINALIZED = allocateSCXRecord(tid);
    ...
    ~bslack() {
        ...
        int nodes = 0;
        int scxRecords = 0;
        set<void*> scxPtrsSeen;
        freeSubtree(entry, &nodes, &scxRecords, &scxPtrsSeen);
