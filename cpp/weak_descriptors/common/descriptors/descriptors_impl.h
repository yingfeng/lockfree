...
#define DESC1_INIT_ALL(numProcesses) { \
    for (int i=0;i<(numProcesses);++i) { \
        DESC1_ARRAY[i].mutables = MUTABLES1_NEW(0); \
    } \
}
...
#define DESC1_SNAPSHOT(descDest, tagptr, sz) ({ \
    ...
    (UNPACK1_SEQ(__src->mutables) == UNPACK1_SEQ((tagptr))); \
})
